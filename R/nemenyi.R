nemenyi <- function(data, conf.int=0.95, sort=TRUE, plottype=c("vline","none","mcb","vmcb","line"), console=FALSE, silent=FALSE, pcol=NULL, title=NULL){
# Friedman and Nemenyi tests for nonparametric comparisons between methods.
#
# Inputs:
#  data - the matrix, including measures of accuracy for several methods (in columns) 
#         for several time series (rows)
...
#     [Default] - vline: Vertical line visualisation 
...
#
# Outputs:
#   out$means - mean rank of each method
#   out$intervals - nemenyi intervals for each method
#   out$fpval - friedman test p-value
#   out$cd - critical distance for nemenyi
#
# Example:
#  N <- 50
#  M <- 4
...
#  
# Ivan Svetunkov, Nikolaos Kourentzes, 2014. v1.3
...
# Sort interval matrix and means
  if(sort==TRUE){
    ...
    ranks.means = ranks.means[order(ranks.means)];
  }
...
# Produce plots
  ...
  if(plottype == "mcb"){
    ...
    axis(1,at=c(1:cols.number),labels=colnames(ranks.intervals));
    axis(2);
  ...
  if(plottype == "vmcb"){
    ...
    # Find max label size
    lbl.size <- nchar(colnames(ranks.intervals))
    ...
    axis(2,at=c(1:cols.number),labels=colnames(ranks.intervals),las=2)
    axis(1);
    box(which="plot", col="black");
